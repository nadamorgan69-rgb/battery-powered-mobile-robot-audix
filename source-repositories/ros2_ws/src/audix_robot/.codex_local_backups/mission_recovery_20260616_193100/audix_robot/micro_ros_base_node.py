...
class AudixMicroRosBase(Node):
    def __init__(self) -> None:
        ...
        self.base_frame_id = str(self.declare_parameter("base_frame_id", "base_footprint").value)
    ...
    def _publish_telemetry(self, data: dict[str, Any], raw: str) -> None:
        ...
        self._publish_odom_and_tf(now, msg)

    @staticmethod
    def _yaw_quaternion(yaw_deg: float) -> tuple[float, float]:
        yaw_rad = math.radians(float(yaw_deg))
        return math.sin(yaw_rad * 0.5), math.cos(yaw_rad * 0.5)

    def _publish_odom_and_tf(self, stamp, telemetry: EspTelemetry) -> None:
        qz, qw = self._yaw_quaternion(telemetry.yaw_deg)
        x_m = float(telemetry.forward_cm) / 100.0
        y_m = float(telemetry.strafe_cm) / 100.0
        ...
        if self.odom_pub is not None:
            ...
            odom.twist.twist.angular.z = math.radians(float(telemetry.gyro_filt_dps))

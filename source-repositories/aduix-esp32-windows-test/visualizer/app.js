...
const pidGroups = [
    ...
    { key: "x", label: "X distance" },
    { key: "y", label: "Y strafe" },
    { key: "yaw", label: "Yaw / Z" },
];
...
function bindUi() {
    ...
    document.getElementById("sendPwmButton").addEventListener("click", () => send(buildPwmCommand("send")));
    document.getElementById("reversePwmButton").addEventListener("click", () => send(buildPwmCommand("reverse")));
    document.getElementById("maxPwmButton").addEventListener("click", () => send(buildPwmCommand("max")));
...
function buildPwmCommand(action) {
    const wheel = document.getElementById("pwmWheel").value;
    const duration = positiveValue("pwmDuration", 1.0);
    if (action === "max") {
        return wheel === "all" ? `motors all max ${fmt(duration)}` : `motor ${wheel} max ${fmt(duration)}`;
    }

    let pwm = clamp(numberValue("pwmValue", 0.3), -1, 1);
    if (action === "reverse") pwm = -Math.abs(pwm);
    return wheel === "all" ? `motors all ${fmt(pwm)} ${fmt(duration)}` : `motor ${wheel} ${fmt(pwm)} ${fmt(duration)}`;
}

...
void printStatus() {
    ...
    consolePrintf(
        "[status] enabled=%s imuHealthy=%s cmd=(%.3f, %.3f, %.3f) limit=%s odom=(x=%.3f y=%.3f theta=%.3f vx=%.3f vy=%.3f wz=%.3f) imu=(yaw=%.3f gz=%.3f) bench=(motion=%s move=%s pwm=%s rpm=%s imu=%s enc=%s rtos=%s)\n",
...
void processCommandTokens(char* line) {
    ...
    if (equalsIgnoreCase(command, "imu")) {
        ...
        consolePrintln("[imu] unavailable in stepper firmware.");
#else
        handleToggle("imu", strtok_r(nullptr, " \t", &context), setBenchImuStreamEnabled);
#endif
        return;
    }
